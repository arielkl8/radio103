...
export default function AudioPlayer({ audioUrl, title, onEnded }: Props) {
  ...
  useEffect(() => {
    ...
    const onLoaded = () => { setDuration(audio.duration); setBuffering(false); };
    const onWaiting = () => setBuffering(true);
    const onPlaying = () => { setPlaying(true); setBuffering(false); };
    const onPause = () => setPlaying(false);
    const onEnded2 = () => { setPlaying(false); setBuffering(false); onEnded?.(); };
  ...
  const togglePlay = async () => {
    ...
    if (!audio) return;
    if (!audio.paused) {
      audio.pause(); // triggers 'pause' event → setPlaying(false)
    } else {
      setBuffering(true);
      try {
        await audio.play(); // triggers 'playing' event → setPlaying(true), setBuffering(false)
      } catch {
        setBuffering(false);
      }
    }
  ...
  return (
    <div className="bg-[#0d1b2a] border border-brand-border rounded-xl p-4 shadow-lg">
      ...
      {/* Progress bar */}
      <div
        className="relative h-2 bg-gray-700 rounded-full cursor-pointer mb-2 group"
        onClick={seek}
      >
        <div
